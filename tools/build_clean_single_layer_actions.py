...
# -*- coding: utf-8 -*-
"""
build_clean_single_layer_actions.py
1. 纯单层独立渲染每一个官方原版 SWF（绝不垫底叠加 Stand.swf，彻底消除重影与多层）
2. 图像形态学精确修补闭合空洞（嘴腔内部暗红、身体内部接缝纯白实心，彻底消除残缺与穿透）
3. 6 帧 64 色高清调色板无损压缩（单帧 ~1.2KB，全库 ~2.5MB）
4. 打包并全量烧录 LittleFS 文件系统镜像
"""
...
def extract_and_pack_clean_swf(page, gender, stage, act_name, swf_rel, num_frames=6, frame_delay=0.08):
    ...
    html_code = HTML_SINGLE_TEMPLATE.format(swf_url=act_url)
    (WORKSPACE / "tools/render_single.html").write_text(html_code, encoding="utf-8")

    page.goto(f"http://127.0.0.1:{PORT}/tools/render_single.html")
    try:
    ...
    for f in range(num_frames):
        ...
        if bbox:
            ...
            # 仅修复内部闭合空洞（嘴腔内部暗红、身躯接缝实心白，绝不动外边缘）
                    ...
                    for x in range(96):
                        if holes[y, x]:
                            if 28 <= y <= 68 and 30 <= x <= 66:
                                arr[y, x] = [150, 40, 45, 255] # 嘴巴暗红实色
                            elif y > 68:
                                arr[y, x] = [240, 240, 240, 255] # 腹部纯白
                            else:
                                arr[y, x] = [30, 35, 45, 255]     # 深色边缘
    ...
    print(f"  [SINGLE CLEAN] {gender}/{stage}/{act_name}.act -> {num_frames} frames ({out_act_file.stat().st_size} bytes)")
    return True

def generate_micro_actions(gender):
    """待机微动作（左右打量、蹒跚摇摆、伸懒腰、雨伞、冷风、小电扇等）"""
    print(f"\n--- Generating micro actions for {gender} ---")
...
def main():
        ...
        for gender in ["MM", "GG"]:
            ...
            for stage, actions in ACTION_CONFIG.items():
                ...
                for act_name, swf_rel, num_f, f_delay in actions:
                    ok = extract_and_pack_clean_swf(page, gender, stage, act_name, swf_rel, num_f, f_delay)
                    if not ok:
    ...
    print(f"\nALL SINGLE-LAYER ACTIONS EXTRACTED! Total Data Size: {total / (1024*1024):.2f} MB ({total / 1024:.1f} KB)")
